...
class Error(Exception):
    def report(self, ctx):
        rep = get_reporter()
        print()
        print(f'{rep.RED}{ctx}:{rep.NORMAL}', *self.args)
        #print(ctx + ':', *self.args)
...
class CommandError(Error):
    ...
    def report(self, ctx):
        rep= get_reporter()
        print()
        print(f'{rep.RED}{ctx}:{rep.NORMAL} error running {rep.YELLOW}{self.cmd[0]!r}{rep.NORMAL} (return code {self.code})')
        if len(self.cmd) > 1:
...
class TestReporter:
    ...
    def __init__(self, **kws):
        ...
        self.NORMAL = curses.tigetstr('sgr0').decode('ascii') or ''
        set_fg = curses.tigetstr('setaf')
        if set_fg:
            self.RED = curses.tparm(set_fg, 1).decode('ascii')
            self.GREEN = curses.tparm(set_fg, 2).decode('ascii')
            self.YELLOW = curses.tparm(set_fg, 3).decode('ascii')
    ...
    def refresh(self):
        if self.show_status:
            if self.errors:
                msg = f'\rCompleted {self.completed_tests} of {self.requested_tests}. {self.YELLOW}Failures {self.failures}. {self.RED}Errors {self.errors}.{self.NORMAL}'
            else:
                msg = f'\rCompleted {self.completed_tests} of {self.requested_tests}. {self.YELLOW}Failures {self.failures}.{self.NORMAL}'
...
class Test:
    ...
    def run(self):
        ...
        try:
            ...
            raise Error("Unable to communicate with program")
        except OSError as e:
            raise Error(f"Error running program: {e}")
        finally:
            timer.cancel()
        ...
        if self.summary:
            ...
            print()
            print(f'{reporter.RED}{self.group}: {reporter.YELLOW}{self.summary}{reporter.NORMAL}')
            ...
            if reporter.show_input:
                self.print_input(reporter)

            if reporter.show_output:
                print()
                print(f'{reporter.YELLOW}output')
                print(f'---{reporter.NORMAL}')
                print(out, end='')
                print(f'{reporter.YELLOW}---{reporter.NORMAL}')
...
class AbstractTestGroup:
    ...
    def __init__(self, id='', weight=1, name=None, category=NORMAL):
        self.id = id
    ...
    def set_project(self, project, program, build_dir, data_dir):
        self.name = f'{project}:{self.name}' if self.name else project
        self.prog = program
...
class StringTests(AbstractTestGroup):
    ...
    def get_tests(self):
        test_file = os.path.join(self.data_dir, self.file)
        ...
        with open(test_file) as lines:
            try:
                while True:
                    ...
                    yield self.test(self.prog, arg, ref=ref)
...
class FileTests(AbstractTestGroup):
    ...
    def get_tests(self):
        ...
        fnames = [fname for fname in os.listdir(self.data_dir)
        ...
        for ref_name in fnames:
            ...
            arg = os.path.join(self.data_dir, arg_name)

            if not os.path.exists(arg):
                logger.warning('Unmatched reference file: %r', ref_name)
                continue

            ref = os.path.join(self.data_dir, ref_name)

            yield self.test(self.prog, arg,
                            input_file = arg,
                            ref_file   = ref)
...
class StdinFileTests(FileTests):
    ...
    def test(self, prog, arg, **kws):
        return super().test(prog, **kws)
...
class Project:
    ...
    def set_context(self, src_dir, build_dir, data_dir, user_dir=None):
        ...
        for group in self.groups:
            group.set_project(self.name, './' + self.prog, build_dir, data_dir)

        for group in self.user_groups:
            group.set_project(self.name, './' + self.prog, build_dir, self.user_dir)
    ...
    def gather_tests(self, requests):
        ...
        for group in self.groups:
            if not requests or f'{self.name}:{group.name}' in requests:
                self.tests.extend(group.get_tests())
...
def test_project(project, src_dir, build_dir, data_dir, fail_stop=False, requests=(), init_only=False):
    ...
    reporter.clear_bar()
    print()
    print(f'{reporter.YELLOW}Tests performed:{reporter.NORMAL} {reporter.completed_tests} of {reporter.requested_tests}')
    print(f'{reporter.YELLOW}Tests failed:{reporter.NORMAL}    {reporter.failures}')
    if reporter.errors:
        print(f'{reporter.RED}Errors:{reporter.NORMAL}          {reporter.errors}')
    ...
    for category,catscores in scores.items():
        ...
        reporter.clear_bar()
        print()
        print(f'{reporter.YELLOW}{category_names[category]}')
        print('-----')
        print(f'  {reporter.YELLOW}{"":{group_width}} Points Failed Score{reporter.NORMAL}')
        for group,score in catscores.items():
            failed       = failures[category][group]
            fcolor       = reporter.RED if failures[category][group] > 0 else ''
            group_points = points[category][group]
            cat_points += group_points
            cat_score  += score

            print(f'  {reporter.YELLOW}{group:{group_width}}{reporter.NORMAL} {group_points:6.1f} {fcolor}{failed:>6} {reporter.GREEN}{score:5.1f}{reporter.NORMAL}')

        if len(catscores) > 1:
            print(f'  {"":{group_width}} ------        -----')
            print(f'  {"":{group_width}} {cat_points:6.1f}        {reporter.GREEN}{cat_score:5.1f}{reporter.NORMAL}')
...
def get_args(src_subdir):
    ...
    argp.add_argument('--nostatus', action='store_true',
        help='Disable the live status bar')
