...
class MLTests(autograde.AbstractTestGroup):
    ...
    def get_tests(self):
        if self.id:
            train_prefix = f'train.{self.id}.'
            data_prefix  = f'data.{self.id}.'
            ref_prefix   = f'ref.{self.id}.'
        ...
        for fname in fnames:
            ...
            ref = os.path.join(self.data_dir, fname)

            data = os.path.join(self.data_dir, data_name)
            if not os.path.exists(data):
                autograde.logger.warning('Missing data file %r', data_name)
                continue

            train = os.path.join(self.data_dir, train_name)
            if not os.path.exists(ref):
                autograde.logger.warning('Missing training file %r', train_name)
                continue

            yield self.test(self.prog, 'train', 'data',
                            ref_file   = ref,
                            train_file = train,
                            data_file  = data)
